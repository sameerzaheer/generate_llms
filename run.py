...
class ScheduledTask:
    def __init__(self, task_id, base_url, time_created=None, time_last_run=None, last_status='pending', last_result=None, 
                 avoid_url_substring_list=None, use_llm=False, llm_instructions=''):
    ...
    def update_last_run(self, status='completed', result=None):
    ...
    def run(self):
        ...
        try:
            generated_llms = create_llms(self.base_url, self.avoid_url_substring_list)
            print(f"Created llms for {self.base_url} at time {datetime.now()} with length {len(generated_llms)}")
            self.update_last_run('completed', generated_llms)
...
class TaskManager:
    ...
    def add_task(self, task_id, base_url, last_result=None, avoid_url_substring_list=None, use_llm=False, llm_instructions=''):
...
def generate():
    ...
    generated_llms = create_llms(url, avoid_list)
    ...
    if schedule_updates:
        ...
        task_manager.add_task(task_id, url, generated_llms, avoid_list, use_llm, llm_instructions)
        print(f"Generated llms.txt for URL: {url} and created scheduled task")
    else:
...
if __name__ == '__main__':
    ...
    print("Scheduled tasks will run every 30 seconds")
